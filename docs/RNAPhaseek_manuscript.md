...
### A leakage-honest benchmark, and the lessons it taught

A recurring hazard in small-corpus sequence prediction is information leakage between near-duplicate sequences. We adopted CD-HIT cluster-grouped cross-validation, in which all sequences sharing ≥90% identity are constrained to the same fold, and we report this as the only honest estimate. The lesson was concrete: an earlier evaluation that grouped only at the per-sequence level reported AUROC 0.847, but ~22% of positives have a ≥95%-identical paralog or fragment; correcting to cluster grouping reduced the honest figure to **0.822**, with fold variance roughly doubling (±0.025 → ±0.054). The leakage was a real but modest ~2.5-point inflation — neither negligible nor, as an adversarial audit initially feared, a collapse. We similarly learned, twice, that small held-out subsets mislead: a non-yeast generalization estimate read 0.71 on a 29-sequence locked test but 0.76–0.80 on the high-powered (n=268) cross-validation, and an organism-balancing intervention appeared to cost overall accuracy on the small test yet was cost-neutral under cross-validation. We therefore base every claim on cluster-grouped cross-validation, not point held-out tests.
...
### Dataset expansion and the yeast-generalization diagnostic

We expanded the strict positive corpus 3.16-fold, from 427 to **1,352** sequences, principally by re-mining the protein-free yeast RNA self-assembly screen of Van Treeck et al. [3] to its full enriched depth (911 net-new, de-leaked), supplemented with a small, verified, anti-fabrication-checked set of non-yeast sequences (archaeal RNase P RNAs, an Arabidopsis G-quadruplex mRNA, designed sticker–spacer and rG4 constructs). The expansion delivered two genuine gains: fold-AUROC variance collapsed ~4-fold (±0.054 → ±0.014), resolving the small-pool confidence-band concern, and structural specificity sharpened. But it also exposed an honest limit through an organism-stratified diagnostic: the model became a strong predictor of **yeast** RNA-self-LLPS (AUROC 0.91) with more moderate transfer to other organisms (0.76). This is volume, not diversity — ~67% of positives are *S. cerevisiae* from a single assay, because strict non-yeast RNA-self-LLPS data barely exists.

We partially closed this with **organism-balanced training**: a sampler that draws non-yeast positives as often as yeast (P(neg)=0.50, P(yeast-pos)=0.25, P(non-yeast-pos)=0.25). Across matched cluster-grouped cross-validation, this lifted non-yeast AUROC **0.763 → 0.798 (+0.036, n=268)** at no cost to overall AUROC (0.883 → 0.884), narrowing the gap ~30% (Figure 5). The final accepted model (RNAPhaseek-v6) combines the expanded corpus, structural negatives, self-complementarity features, cluster-grouped leakage control, and organism-balanced sampling, retrained on all data: overall AUROC **0.884**, structural-specificity **0.898**, yeast **0.909**, non-yeast **0.798**.

### De novo design of structurally-grounded self–phase-separating RNA

Using the v6 model as a fitness oracle, we implemented three generators (Methods): gradient design (Fast SeqProp), a genetic algorithm (GA) scoring candidates through the complete pipeline, and a diversity-penalized deep exploration network (DEN). To distinguish genuine designs from composition artifacts, we introduced a **structure-dependence** test — the difference between a sequence’s predicted P(LLPS) and that of its composition-matched scramble (Δ). The GA converged to a single, maximally-optimized motif (P=0.996) that is the most structure-grounded set produced (Δ=+0.52; score 0.995 collapsing to 0.48 when scrambled) — exceeding even real LLPS positives (Δ=+0.28). The DEN produced a diverse library (15 distinct designs, mean pairwise identity 0.46 vs. the GA’s 0.82) that remains as structure-dependent as real positives (Δ=+0.28), trading per-design optimization for breadth (Figure 6). Random RNA, as expected, is structure-independent (Δ=−0.13) and scored near zero. The two generators are complementary: the GA supplies a single high-confidence anchor, the DEN a diverse panel that the structure-dependence test can triage. These are model-believed candidates awaiting experimental validation.

### A usable tool

RNAPhaseek is released as a single command-line program with three subcommands — `score` (P(LLPS) per FASTA), `design` (GA or DEN generation), and `validate` (structure-dependence triage). Prediction requires only the model and computes structural features on the fly; the full training cache is not needed for inference.
...
**Architecture and features.** The backbone is RNA-FM (multimolecule/rnafm) [22], final two layers fine-tuned; a 2-layer FEGSTrans adapter applies a FEGS-derived structural encoding as graph-bias attention [FEGS ref — verify]. Sequences longer than 1,022 nt are windowed; biophysical features (38-dim, including five self-complementarity features computed over an 800-nt window with ViennaRNA [23] where applicable) are computed per RNA and fused after attention pooling.

**Training and evaluation.** Models were trained with AdamW, early stopping on an inner-validation AUROC, label smoothing, and a class-and-organism-balanced WeightedRandomSampler (final model: P(neg)=0.50, P(yeast-pos)=0.25, P(non-yeast-pos)=0.25). Evaluation used 5-fold StratifiedGroupKFold with CD-HIT clusters as groups (and structural negatives following their parent), a grouped held-out locked test, and the frozen external nanostar set. A feature-ablation control (zeroing the self-complementarity features) and a determinism canary (asserting identical featurization at train and test time) were applied. The final accepted model was retrained on all data (1,973 of 2,177 sequences; 10% grouped inner-validation for early stopping); its performance is characterized by the leakage-free cross-validation.
...
The RNAPhaseek model, code, strict corpus, and candidate designs are available at [repository URL — to be added]. The final model and a one-command command-line tool (`rnaphaseek`, with `score`/`design`/`validate` subcommands) are provided for prediction and design. The curated strict RNA-self-LLPS corpus is provided in FASTA format with provenance annotations.
...
**Figure 2.** Leakage-honest performance. CV AUROC progression under per-sequence vs. CD-HIT cluster grouping (0.847 → 0.822 honest baseline), and the v6 expansion (→0.884) with ~4× lower fold variance.

**Figure 3.** Structural specificity. Distinguishing condensing positives from composition-matched, structure-destroyed negatives improves from ~0.50 (composition-only) to 0.90 (v6, cross-validated).
