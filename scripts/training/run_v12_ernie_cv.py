"""v12 ERNIE-RNA CV — v6's organism-balanced 5-fold cluster-grouped CV with the BACKBONE swapped
from RNA-FM to ERNIE-RNA (multimolecule/ernierna, 86M, base-pairing attention prior). IDENTICAL
pool (v5), cluster groups, folds, seeds, sampler, and recipe as v6 — only the backbone differs, so
v12-vs-v6 isolates the backbone effect. The 18 v11 G4 benchmark seqs are NOT in this pool, so the
structure-specificity benchmark stays a clean held-out test of the backbone.

Reports overall / struct-specificity / yeast / non-yeast OOF AUROC vs v6 (0.884/0.898/0.909/0.798).
Resumable.

  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python scripts/training/run_v12_ernie_cv.py
"""
import os, sys, json
...
OUT = "model/strict_eval_v12_ernie_cv"
SP = "Data/splits"
...
def train_orgbalanced(tr_idx, va_idx, aug, yeast, args, device, tok, tag):
    """Copy of run_v6_cv.train_orgbalanced + a NON-FINITE-GRADIENT SKIP guard: ERNIE-RNA's recursive
    bias overflows to NaN on rare long sequences; the forward nan_to_num keeps the loss finite, but the
    backward still routes NaN grads through the unfrozen backbone. Skipping opt.step() when the grad
    norm is non-finite stops a single bad batch from poisoning the weights (which caused all-NaN val)."""
    y = E.G["y"]
    ...
    for ep in range(args.epochs):
        ...
        for tk, at, Lh, bi, yb in train_loader:
            ...
            if not torch.isfinite(loss):
                skipped += 1; continue
            loss.backward()
            # Robust guard: a blanked (overflow->0) sequence makes adapter LayerNorm backward produce
            # NaN grads. Zero ONLY the non-finite grad entries and still step, so the batch's good
            # gradients (other params/examples) keep training — vs all-or-nothing skip which stalled folds.
            bad = False
            for p in model.parameters():
                if p.grad is not None and not torch.isfinite(p.grad).all():
                    torch.nan_to_num_(p.grad, nan=0.0, posinf=0.0, neginf=0.0); bad = True
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step(); sched.step()
            if bad:
                skipped += 1            # count batches that needed grad-cleaning (diagnostic)
        ...
        if mv["auroc"] > best_au + 1e-4:
            ...
            pat += 1
        print(f"    [{tag}] ep{ep+1}/{args.epochs} val={mv['auroc']:.4f} (best={max(best_au,0):.4f} pat {pat}/{args.patience}) skipped={skipped}", flush=True)
...
def main():
    ...
    is_struct = build_pool_v5()                                   # SAME v5 pool as v6
    groups = np.load(f"{SP}/cluster_groups_v5.npy"); yeast = np.load(f"{SP}/is_yeast_v5.npy")
    y = E.G["y"]; N = len(y); all_idx = np.arange(N)
    # ERNIE-RNA backbone FROZEN: this multimolecule build overflows to NaN on long seqs, and the NaN
    # backward poisons any UNFROZEN backbone params on nearly every batch. Freezing removes the backbone
    # grad path entirely (stable); the adapter/head learn on ERNIE's guard-cleaned frozen representation
    # (which the frozen probe showed separates the matched G4 pairs ~10x). NOTE vs v6: v6 unfreezes 2
    # RNA-FM layers, so this is frozen-ERNIE vs finetuned-RNA-FM — a recipe caveat, documented.
    ...
    for k, (otr, ova) in enumerate(sgkf.split(dev_idx, y[dev_idx], groups[dev_idx])):
        ...
        model, m, sd = train_orgbalanced(tr, va, aug, yeast, args, device, tok, f"v12ernie{k}")
    ...
    print(f"\n*** v12 ERNIE-RNA CV (backbone swap; same v5 pool/protocol as v6) ***", flush=True)
    ...
    json.dump({"cv_oof_auroc": float(oau), "cv_struct_auroc": sau, "cv_yeast_auroc": yau, "cv_nonyeast_auroc": nyau,
               "cv_nonyeast_n": nnyp, "cv_fold_scores": cv, "backbone": BACKBONE,
               "v6_cv_oof": v6["cv_oof_auroc"], "v6_cv_struct": v6["cv_struct_auroc"], "v6_cv_nonyeast": v6["cv_nonyeast_auroc"],
               "note": "ERNIE-RNA backbone, same v5 pool/groups/folds/seeds/sampler as v6 (run_v6_cv.train_orgbalanced)"},
