"""v10 CV — v6's organism-balanced 5-fold cluster-grouped CV, repointed at the CLEANED +
EXPANDED v10 pool: protein-dependent positives removed (38 lncRNA clients), r_CUU31 relabeled
POS->NEG, scrambled-CAG soft-negatives excluded, +17 new protein-free sequences (matched
negatives + non-yeast positives). Same protocol/seeds as v6. Reports overall / struct-specificity
/ yeast / non-yeast OOF AUROC vs v6 (0.884 / 0.898 / 0.909 / 0.798). Resumable.

  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python scripts/training/run_v10_cv.py
"""
import os, sys, json
...
def build_pool_v10():
    ...
    pp = list_npz_sorted("Data/processed/fegs_v10_pos")
    npn = list_npz_sorted("Data/processed/fegs_v10_neg")
    ...
    E.G["bio"] = np.vstack([np.load(f"{SP}/biophys_v10_pos.npy"),
                            np.load(f"{SP}/biophys_v10_neg.npy"),
                            np.load(f"{SP}/biophys_v4_structneg.npy")]).astype(np.float32)
    is_struct = np.array(["hardneg_struct" in h for h in E.G["hdrs"]])
    print(f"v10 pool: {npos} pos / {nneg} real-neg / {nsn} struct-neg = {len(E.G['y'])}", flush=True)
    return is_struct


def main():
    set_seed(42); device = setup_device(); os.makedirs(OUT, exist_ok=True)
    is_struct = build_pool_v10()
    groups = np.load(f"{SP}/cluster_groups_v10.npy"); yeast = np.load(f"{SP}/is_yeast_v10.npy")
    assert len(groups) == len(E.G["y"]) == len(yeast), "split arrays misaligned with v10 pool"
    ...
    for k, (otr, ova) in enumerate(sgkf.split(dev_idx, y[dev_idx], groups[dev_idx])):
        ...
        model, m, sd = train_orgbalanced(tr, va, aug, yeast, args, device, tok, f"v10cv{k}")
    ...
    print(f"\n*** v10 CV (cleaned + expanded corpus: 1321 pos / 648 neg) ***", flush=True)
    print(f"  overall OOF = {oau:.4f}   (v6 = {v6['cv_oof_auroc']:.4f})")
    print(f"  STRUCT-spec = {sau:.4f}   (v6 = {v6['cv_struct_auroc']:.4f})   <- cleaner labels should help most here")
    ...
    json.dump({"cv_oof_auroc": float(oau), "cv_struct_auroc": sau, "cv_yeast_auroc": yau, "cv_nonyeast_auroc": nyau,
               ...
               "pool": "v10 cleaned+expanded (1321 pos / 648 neg); removed 38 protein-dependent, fixed CUU31, excluded scrambled-CAG"},
