"""v13 CV — v6's organism-balanced 5-fold cluster-grouped CV, repointed at the v13 pool
...
v13-vs-v6 isolates the effect of the additions. Reports overall / struct-specificity / yeast /
non-yeast OOF AUROC vs v6 (0.884 / 0.898 / 0.909 / 0.798). Resumable.

  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python scripts/training/run_v13_cv.py
"""
import os, sys, json
...
def build_pool_v13():
    ...
    pp = list_npz_sorted("Data/processed/fegs_v13_pos")
    npn = list_npz_sorted("Data/processed/fegs_v13_neg")
    ...
    E.G["bio"] = np.vstack([np.load(f"{SP}/biophys_v13_pos.npy"),
                            np.load(f"{SP}/biophys_v13_neg.npy"),
...
def main():
    ...
    is_struct = build_pool_v13()
    groups = np.load(f"{SP}/cluster_groups_v13.npy"); yeast = np.load(f"{SP}/is_yeast_v13.npy")
    assert len(groups) == len(E.G["y"]) == len(yeast), "split arrays misaligned with v13 pool"
    ...
    for k, (otr, ova) in enumerate(sgkf.split(dev_idx, y[dev_idx], groups[dev_idx])):
        ...
        model, m, sd = train_orgbalanced(tr, va, aug, yeast, args, device, tok, f"v13cv{k}")
    ...
    print(f"\n*** v13 CV (ADDITIVE corpus: v5/v6 pool + 18 verified protein-free additions) ***", flush=True)
    ...
    json.dump({"cv_oof_auroc": float(oau), "cv_struct_auroc": sau, "cv_yeast_auroc": yau, "cv_nonyeast_auroc": nyau,
               ...
               "pool": "v13 additive (v5/v6 pool + 18 verified protein-free additions: RNA-G4 LLPS + matched negs + 2CZ aptamer)"},
