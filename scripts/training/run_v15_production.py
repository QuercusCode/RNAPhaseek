"""v15 production — v6 recipe (RNA-FM + FEGS adapter, unfreeze 2, 38-dim biophysics, organism-
balanced sampler) trained on the v15 pool = v5 + de-leaked matched TRAINING pairs. Tests the
...
showed a fancy backbone isn't the lever). Saves a self-contained checkpoint for the benchmark.
  ...
  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python scripts/training/run_v15_production.py
"""
import os, sys, json
...
OUT = "model/strict_eval_v15_production"
SP = "Data/splits"
...
def build_pool_v15():
    ...
    print(f"v15 pool: {npos} pos / {nneg} real-neg / {nsn} struct-neg = {len(E.G['y'])}", flush=True)
...
def main():
    ...
    print(f"v15 production: train={len(tr_idx)} (~90% of {N}) val={len(va_idx)}", flush=True)
    ...
    DW, MATCH_TAG = 0.40, "v15_synth"
    matched = np.array([MATCH_TAG in E.G["hdrs"][i] for i in tr_idx] + [False] * len(aug["y"]))
    w = w * np.where(matched, DW, 1.0)
    print(f"  downweighting {int(matched.sum())} synthetic matched pairs x{DW}", flush=True)
    ...
    for ep in range(args.epochs):
        ...
        if mv["auroc"] > best_au + 1e-4:
            ...
            pat += 1
        print(f"    [v15-prod] ep{ep+1}/{args.epochs} inner_val={mv['auroc']:.4f} (best={max(best_au,0):.4f} pat {pat}/{args.patience})", flush=True)
        if pat >= args.patience:
            print(f"    [v15-prod] early stop @ ep{ep+1}", flush=True); break
    if best_state is not None: model.load_state_dict(best_state)
    torch.save(model.state_dict(), f"{OUT}/final_model.pt")
    np.savez(f"{OUT}/norm_stats.npz", mean=m, std=sd)
    json.dump({"model": "v15 (v6 recipe + de-leaked matched training pairs)", "backbone": args.backbone,
    ...
    print(f"\n*** v15 model saved -> {OUT}/final_model.pt (best inner-val {best_au:.4f}) ***", flush=True)
