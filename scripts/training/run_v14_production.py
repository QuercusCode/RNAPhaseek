"""v14 production — v6 recipe (RNA-FM + FEGS adapter, unfreeze 2, 38-dim biophysics, organism-
balanced sampler) trained on the v14 pool = v5 + de-leaked matched TRAINING pairs. Tests the
...
showed a fancy backbone isn't the lever). Saves a self-contained checkpoint for the benchmark.
  ...
  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python scripts/training/run_v14_production.py
"""
import os, sys, json
...
OUT = "model/strict_eval_v14_production"
SP = "Data/splits"
TARGET = {0: 0.50, 1: 0.25, 2: 0.25}


def build_pool_v14():
    pos = read_fasta("Data/raw/multispecies/strict_pool_v14_positives.fasta")
    neg = read_fasta("Data/raw/multispecies/strict_pool_v14_negatives_all.fasta")
    sneg = read_fasta("Data/raw/multispecies/strict_struct_negatives_v4.fasta")
    pp = list_npz_sorted("Data/processed/fegs_v14_pos"); npn = list_npz_sorted("Data/processed/fegs_v14_neg")
    ...
    E.G["bio"] = np.vstack([np.load(f"{SP}/biophys_v14_pos.npy"), np.load(f"{SP}/biophys_v14_neg.npy"),
                            np.load(f"{SP}/biophys_v4_structneg.npy")]).astype(np.float32)
    print(f"v14 pool: {npos} pos / {nneg} real-neg / {nsn} struct-neg = {len(E.G['y'])}", flush=True)


def main():
    set_seed(42); device = setup_device(); os.makedirs(OUT, exist_ok=True)
    build_pool_v14()
    groups = np.load(f"{SP}/cluster_groups_v14.npy"); yeast = np.load(f"{SP}/is_yeast_v14.npy")
    ...
    print(f"v14 production: train={len(tr_idx)} (~90% of {N}) val={len(va_idx)}", flush=True)
    ...
    for ep in range(args.epochs):
        ...
        if mv["auroc"] > best_au + 1e-4:
            ...
            pat += 1
        print(f"    [v14-prod] ep{ep+1}/{args.epochs} inner_val={mv['auroc']:.4f} (best={max(best_au,0):.4f} pat {pat}/{args.patience})", flush=True)
        if pat >= args.patience:
            print(f"    [v14-prod] early stop @ ep{ep+1}", flush=True); break
    if best_state is not None: model.load_state_dict(best_state)
    torch.save(model.state_dict(), f"{OUT}/final_model.pt")
    np.savez(f"{OUT}/norm_stats.npz", mean=m, std=sd)
    json.dump({"model": "v14 (v6 recipe + de-leaked matched training pairs)", "backbone": args.backbone,
    ...
    print(f"\n*** v14 model saved -> {OUT}/final_model.pt (best inner-val {best_au:.4f}) ***", flush=True)
