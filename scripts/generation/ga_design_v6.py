...
def main():
    ...
    def score_batch(seqs):
        ...
        d = Path(tempfile.mkdtemp(prefix="fegs_gav6_"))
        fa = d / "s.fasta"
        with open(fa, "w") as f:
    ...
    with open("outputs/designs/designed_ga_v6.fasta", "w") as f:
    ...
    fig.savefig("report_assets/fig21_ga_v6_convergence.png", dpi=140, bbox_inches="tight"); plt.close()
    json.dump({"history": history, "top": [{"seq": s, "P": cache[s]} for s in top]},
              open("model/strict_eval_v4/ga_v6_summary.json", "w"), indent=2)
    print("\nSaved -> designed_ga_v6.fasta, report_assets/fig21_ga_v6_convergence.png")
