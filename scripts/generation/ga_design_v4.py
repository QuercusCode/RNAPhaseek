"""
Genetic-algorithm de novo design against the STRUCTURE-AWARE v4 model.

Identical hyperparameters to the v3 GA (ga_design.py) for a clean comparison, but
fitness is the full v4 pipeline (38-dim biophysics incl. the 5 self-complementarity
features + FEGS + forward). Because v4 rewards self-complementarity, the GA should
evolve structurally-grounded designs — verified afterwards by the design-vs-scramble
structure-dependence check.

  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python ga_design_v4.py
"""
...
def main():
    ...
    def score_batch(seqs):
        ...
        d = Path(tempfile.mkdtemp(prefix="fegs_gav4_"))
        fa = d / "s.fasta"
        with open(fa, "w") as f:
    ...
    top = final[:10]
    print("\n=== TOP v4-GA DESIGNS (full v4-model P(LLPS)) ===")
    ...
    with open("outputs/designs/designed_ga_v4.fasta", "w") as f:
        for i, s in enumerate(top): f.write(f">ga_v4_design_{i}_P{cache[s]:.3f}\n{s}\n")
    h = np.array(history)
    ...
    ax.set_xlabel("generation"); ax.set_ylabel("full v4-model P(LLPS)")
    ax.set_title("Figure 17 — Genetic-algorithm convergence on the structure-aware v4 model", fontweight="bold")
    ax.legend(frameon=False); ax.grid(alpha=0.25, ls="--")
    fig.savefig("report_assets/fig17_ga_v4_convergence.png", dpi=140, bbox_inches="tight"); plt.close()
    json.dump({"history": history, "top": [{"seq": s, "P": cache[s]} for s in top]},
              open("model/strict_eval_v4/ga_v4_summary.json", "w"), indent=2)
    print("\nSaved -> designed_ga_v4.fasta, report_assets/fig17_ga_v4_convergence.png")
