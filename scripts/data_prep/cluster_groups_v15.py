"""Leakage-honest CD-HIT cluster grouping + is_yeast for the v15 pool (v5 + matched pairs).
...
The new matched pairs (synthetic, non-yeast) cluster among themselves by their shared G4 cores.
  ...
  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python scripts/data_prep/cluster_groups_v15.py
"""
...
def main():
    ...
    print(f"v15 pool: {npos} pos / {nneg} real-neg / {nsn} struct-neg")

    d = Path(tempfile.mkdtemp(prefix="cdhit_v15_"))
    fa = d / "posneg.fasta"
    with open(fa, "w") as f:
    ...
    np.save("Data/splits/cluster_groups_v15.npy", groups)
    np.save("Data/splits/is_yeast_v15.npy", yeast)
    ...
    print("Saved -> cluster_groups_v15.npy, is_yeast_v15.npy")
