"""Leakage-honest cluster grouping for the v11 pool (v5 + 18 additions). Mirrors
cluster_groups_v10 but v11 is PURELY ADDITIVE: every struct-neg parent (a v5 positive) is still
present, so no orphans are expected. The new G4 variants share the (G3A2)4 core / flanks and will
CD-HIT-cluster together (and with their matched neg flanks) → matched pos/neg pairs land in the
same fold, exactly the leakage-honesty we want. Canonical generator of is_yeast_v11.

Pool order (must match the v11 trainer): positives(v5+10) + real negatives(v5+8) + struct negs(184).
  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python scripts/data_prep/cluster_groups_v11.py
"""
...
def main():
    ...
    v5pos = read_fasta("Data/raw/multispecies/strict_pool_v5_positives.fasta")   # parent->seq lookup
    npos, nneg, nsn = len(pos), len(neg), len(sneg)
    print(f"v11 pool: {npos} pos / {nneg} real-neg / {nsn} struct-neg")

    d = Path(tempfile.mkdtemp(prefix="cdhit_v11_"))
    fa = d / "posneg.fasta"
    with open(fa, "w") as f:
    ...
    seq2cl = {nrm(s): cl[i] for i, (_, s) in enumerate(pos)}     # v11-positive seq -> cluster
    ...
    for k, (h, _) in enumerate(sneg):
        ...
        else:                                       # parent absent -> isolate (not expected for additive v11)
            groups[n_real + k] = next_cl; next_cl += 1; orphan += 1
        yeast[n_real + k] = is_yeast(parent_hdr)

    np.save("Data/splits/cluster_groups_v11.npy", groups)
    np.save("Data/splits/is_yeast_v11.npy", yeast)
    ...
    print(f"non-yeast NEGATIVES: {int((~yeast[npos:npos+nneg]).sum())}/{nneg}")
    print("Saved -> cluster_groups_v11.npy, is_yeast_v11.npy")
