"""Leakage-honest CD-HIT cluster grouping + is_yeast for the v14 pool (v5 + matched pairs).
...
The new matched pairs (synthetic, non-yeast) cluster among themselves by their shared G4 cores.
  ...
  /opt/homebrew/Caskroom/mambaforge/base/envs/rnaphaseek/bin/python scripts/data_prep/cluster_groups_v14.py
"""
...
def main():
    ...
    print(f"v14 pool: {npos} pos / {nneg} real-neg / {nsn} struct-neg")

    d = Path(tempfile.mkdtemp(prefix="cdhit_v14_"))
    fa = d / "posneg.fasta"
    with open(fa, "w") as f:
    ...
    np.save("Data/splits/cluster_groups_v14.npy", groups)
    np.save("Data/splits/is_yeast_v14.npy", yeast)
    ...
    print("Saved -> cluster_groups_v14.npy, is_yeast_v14.npy")
